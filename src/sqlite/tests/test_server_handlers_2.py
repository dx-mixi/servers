...
async def test_handle_call_tool():
    ...
    describe_table_result = await call_tool_handler("describe_table", {"table_name": "test"})
    ...
    append_insight_result = await call_tool_handler("append_insight", {"insight": "テストインサイト"})
    ...
    read_query_result = await call_tool_handler("read_query", {"query": "SELECT * FROM test"})
    ...
    write_query_result = await call_tool_handler("write_query", {"query": "INSERT INTO test VALUES (1)"})
    ...
    with pytest.raises(ValueError, match="Only CREATE TABLE statements are allowed"):
        await call_tool_handler("create_table", {"query": "INSERT INTO test VALUES (1)"})
    
    create_table_result = await call_tool_handler("create_table", {"query": "CREATE TABLE new_table (id INT)"})
