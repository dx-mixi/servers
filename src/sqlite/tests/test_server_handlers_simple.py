...
async def test_handle_call_tool():
    ...
    with patch("src.mcp_server_sqlite.server.db", mock_db):
        ...
        describe_table_result = await handle_call_tool(
        ...
        with patch("src.mcp_server_sqlite.server.server") as mock_server:
            ...
            append_insight_result = await handle_call_tool(
        ...
        with pytest.raises(ValueError, match="Only SELECT queries are allowed"):
            await handle_call_tool(
                "read_query", {"query": "INSERT INTO test VALUES (1)"}
            )

        read_query_result = await handle_call_tool(
        ...
        write_query_result = await handle_call_tool(
        ...
        with pytest.raises(
            ValueError, match="Only CREATE TABLE statements are allowed"
        ):
            await handle_call_tool(
                "create_table", {"query": "INSERT INTO test VALUES (1)"}
            )

        create_table_result = await handle_call_tool(
